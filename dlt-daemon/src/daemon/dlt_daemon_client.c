                           ...
                           int verbose)
{
    ...
    // 如果当前状态不是SEND_BUFFER，并且不是强制发送模式，则将消息写入离线跟踪；
    if ((sock != DLT_DAEMON_SEND_FORCE) && (daemon->state != DLT_DAEMON_STATE_SEND_BUFFER)) {
        ...
        // 如果消息头部信息不为空，则将消息写入离线日志存储；如果离线日志存储写入失败，则记录错误日志
    ...
    // 如果当前模式为外部或两者，并且当前状态为直接发送或强制发送，则将消息转发到网络客户端；如果转发失败并且是强制发送模式，则返回发送失败错误码
...
#endif
        // direct模式下，消息直接发送到客户端；force模式下，无论当前状态如何，都强制发送消息到客户端
    ...
    // buffer模式下，如果当前状态为BUFFER、SEND_BUFFER或BUFFER_FULL，
    // 并且不是强制发送模式，则将消息存储在环形缓冲区中；如果缓冲区已满，则丢弃消息并增加丢弃计数器；如果之前有消息被丢弃，并且现在有客户端连接了，则尝试发送一条溢出消息通知客户端，并重置丢弃计数器
    if ((sock != DLT_DAEMON_SEND_FORCE) &&
        ...
        if (daemon->state != DLT_DAEMON_STATE_BUFFER_FULL) {
            ...
            // 将消息存储在环形缓冲区中，如果存储失败则将状态改为BUFFER_FULL
...
// 根据控制消息的ID进行处理，如果是网关模式还需要判断是否需要转发消息
                                      ...
                                      int verbose)
{
    ...
    // 根据Service ID进行处理
    // ID范围在DLT_SERVICE_ID和DLT_SERVICE_ID_CALLSW_CINJECTION之间的消息为控制消息
