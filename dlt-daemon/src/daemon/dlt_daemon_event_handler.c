 ...
 */
// 处理事件的函数，等待所有连接上的事件发生，一旦事件发生，调用特定连接的回调函数，或者如果发生挂起则销毁连接
                            ...
                            DltDaemonLocal *daemon_local)
{
    ...
    // 调用poll函数等待事件发生，超时时间为DLT_EV_TIMEOUT_MSEC毫秒，如果发生错误则记录日志并返回错误码
    ret = poll(pEvent->pfd, pEvent->nfds, DLT_EV_TIMEOUT_MSEC);

    // 如果poll函数返回值小于等于0，说明发生了错误或者超时，
    // 如果是被信号中断则将返回值设为0，否则记录错误日志并返回错误码
    if (ret <= 0) {
    ...
    // 遍历所有的文件描述符，检查是否有事件发生，
    // 如果有事件发生则根据文件描述符找到对应的连接，并调用对应的回调函数进行处理，
    // 如果发生错误则根据连接类型进行相应的处理
    for (i = 0; i < pEvent->nfds; i++) {
        ...
        // 如果连接存在且有接收器，则获取连接类型和接收器的文件描述符，否则禁用该文件描述符并继续处理下一个事件
        if (con && con->receiver) {
            type = con->type;
            fd = con->receiver->fd;
        }
        ...
        // 如果发生了错误事件，根据连接类型进行相应的处理，如果是TCP客户端连接则调用专门的函数进行处理，否则调用取消注册连接的函数进行处理，然后继续处理下一个事件
        ...
        // 根据连接类型获取 对应的回调函数 ，如果无法找到则记录错误日志并继续处理下一个事件
        ...
        // 调用回调函数处理事件，如果处理失败则记录错误日志并返回错误码
        if (callback(daemon,
                     daemon_local,
                     con->receiver,
...
// 如果配置了系统守护进程看门狗功能，则在处理完事件后触发看门狗
