...
/* dlt_init() 完成了 DLT 客户端库的完整初始化，包括连接建立、线程管理、共享资源分配等工作 */
DltReturnValue dlt_init(void)
{
    /* process is exiting. Do not allocate new resources. */
    // 进程状态检查：检查进程是否正在退出（dlt_user_freeing），如果是则禁用日志并返回
     ...
     * */
    // 初始化状态检查：使用原子操作比较和交换（atomic_compare_exchange_strong）检查初始化状态（dlt_user_init_state）
    // 如果状态是INIT_UNITIALIZED，则将其设置为INIT_IN_PROGRESS并继续初始化过程；
    // 如果不是，则说明已经有线程在进行初始化，当前线程直接返回DLT_RETURN_OK。
    ...
    // 环境变量检查：调用dlt_check_envvar()函数检查环境变量的设置，以确定日志记录的配置和行为
    dlt_check_envvar();

    /* Check logging mode and internal log file is opened or not*/
    // 日志模式检查：检查当前的日志记录模式（logging_mode）是否设置为文件模式DLT_LOG_TO_FILE，
    // 并且内部日志文件句柄（logging_handle）是否为NULL。
    ...
    // 公共初始化：调用dlt_init_common()函数执行DLT库的公共初始化步骤，
    // 如果返回错误，则设置初始化状态为INIT_ERROR，释放资源并返回错误代码。
    ...
    // 文件相关初始化：将dlt_user结构体中的dlt_is_file成员设置为0，表示默认情况下不使用文件进行日志记录。
    dlt_user.dlt_is_file = 0;
    dlt_user.filesize_max = UINT_MAX;
    dlt_user_file_reach_max = false;

    // 溢出相关初始化：将dlt_user结构体中的overflow成员设置为0，表示当前没有发生日志溢出；
    dlt_user.overflow = 0;
    dlt_user.overflow_counter = 0;

// 共享内存初始化：初始化 DLT 共享内存连接
// 清零dlt_user结构体中的dlt_shm成员，并调用dlt_shm_init_client()函数初始化共享内存客户端。
...
#endif
// Trace load 设置：分配并初始化 trace load 限制设置
// 如果启用了 DLT_TRACE_LOAD_CTRL_ENABLE 功能，使用写锁保护 trace_load_settings 结构体的分配和初始化过程。
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
...
#endif
// IPC连接初始化：根据不同的 IPC 机制（UNIX socket、VSOCK 或 FIFO）初始化与 DLT 守护进程的连接。
#ifdef DLT_LIB_USE_UNIX_SOCKET_IPC
...
// 网络跟踪初始化：初始化分段消息队列的读写句柄，并初始化相关的条件变量。
#ifdef DLT_NETWORK_TRACE_ENABLE
    ...
    // 启动后台线程：调用 dlt_start_threads() 启动管家线程和其他必要的线程（如网络跟踪线程）
    // 调用dlt_start_threads()函数启动必要的线程（如守护线程），
    // 如果失败则设置初始化状态为INIT_ERROR，释放资源并返回错误代码。
    ...
    // Fork 处理：注册 fork 处理程序以确保安全的进程分叉
    // 调用pthread_atfork()函数注册一个子进程的处理程序（dlt_fork_child_fork_handler），
    // 以确保在调用fork()时正确处理DLT库的状态。
    pthread_atfork(NULL, NULL, &dlt_fork_child_fork_handler);

    // 状态更新：原子地设置初始化状态为 INIT_DONE，表示初始化完成。
                                                       ...
                                                       int32_t args_num)
{
 ...
 // This function is used for v1: DLT_HTYP_PROTOCOL_VERSION1 (协议版本1)
 // where the buffer is managed internally and should be freed after sending the log message.
...
// This function is used for v2: DLT_HTYP2_PROTOCOL_VERSION2 (协议版本2), 
// which supports verbose mode and extended header,
// where the buffer is managed internally and should be freed after sending the log message.
...
// 通用的日志数据写入函数，用于写入带有可选属性信息的数据
/*
log：日志上下文数据
datap：实际要写入的数据指针
datalen：数据长度
type_info：数据类型信息（如 UINT、FLOA 等）
varinfo：可选的变量信息（名称和单位）
*/
static DltReturnValue dlt_user_log_write_generic_attr(DltContextData *log, const void *datap, size_t datalen, uint32_t type_info, const VarInfo *varinfo)
{
    ...
    // 缓冲区大小检查：计算写入数据所需的总大小，并检查是否超过用户缓冲区的长度
    size_t needed_size = datalen;
    if ((size_t)log->size + needed_size > dlt_user.log_buf_len)
        return DLT_RETURN_USER_BUFFER_FULL;

    // 如果处于详细模式，计算额外的类型信息和变量信息所需的空间，并检查总大小是否超过缓冲区长度
    if (is_verbose_mode(dlt_user.verbose_mode, log)) {
    ...
    //  实际数据 (datalen 字节)
    // Verbose 模式下缓冲区结构：
    //  类型信息 (4 字节) + [名称长度 (2 字节) + 名称字符串 + 单位长度 (2 字节) + 单位字符串] + 实际数据 (datalen 字节)
...
// This function is used to send log messages through the DLT user library
DltReturnValue dlt_user_log_send_log(DltContextData *log, const int mtype, int *const sent_size)
{
    ...
    // 检查DLT库是否已初始化且未处于释放状态，确保在正确的状态下发送日志消息
    if (!DLT_USER_INITIALIZED_NOT_FREEING) {
        dlt_vlog(LOG_WARNING, "%s dlt_user_init_state=%i (expected INIT_DONE), dlt_user_freeing=%i\n", __func__, dlt_user_init_state, dlt_user_freeing);
        return DLT_RETURN_ERROR;
    }

    // 通过互斥锁保护对DLT用户库的访问，确保线程安全
    dlt_mutex_lock();
    // 验证日志上下文和消息类型的有效性，确保日志消息具有正确的上下文和类型
    if ((log == NULL) ||
        (log->handle == NULL) ||
    ...
    // 设置用户消息头，指定消息类型为日志消息，以便正确处理和发送日志消息
    ...
    // 设置存储头，指定ECU ID等信息，以便正确标识和处理日志消息
    ...
    // 设置标准头，指定协议版本、ECU ID、时间戳、会话 ID 等信息，以便正确标识和处理日志消息
    ...
    // 根据当前的详细模式设置标准头的类型字段，决定是否包含扩展头，以便在不同模式下正确处理日志消息
    if (is_verbose_mode(dlt_user.verbose_mode, log))
...
#endif
    // 设置消息计数器，确保每条日志消息都有唯一的标识，以便正确跟踪和处理日志消息
    msg.standardheader->mcnt = log->handle->mcnt++;

    /* Set header extra parameters */
    // 设置标准头的额外参数，如ECU ID、时间戳等，以便正确标识和处理日志消息
    dlt_set_id(msg.headerextra.ecu, dlt_user.ecuID);

    /*msg.headerextra.seid = 0; */
    // 根据用户配置设置时间戳，如果配置为自动时间戳，则使用系统的运行时间作为时间戳；
    // 否则，使用用户指定的时间戳，以便在不同配置下正确处理日志消息
...
// 如果启用了负载控制跟踪功能，则将时间戳存储在本地变量中，以便后续使用和处理日志消息
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
    ...
    if (DLT_IS_HTYP_UEH(msg.standardheader->htyp)) {
        ...
        case DLT_TYPE_LOG:
        {
            // 根据日志消息的类型设置扩展头的MSIN字段，指定消息类型和日志级别，以便正确标识和处理日志消息
        ...
        {
            // 根据网络跟踪消息的类型设置扩展头的MSIN字段，指定消息类型和跟踪状态，以便正确标识和处理网络跟踪消息
        ...
        // 设置扩展头的其他字段，如参数数量、应用程序ID、上下文ID等，以便正确标识和处理日志消息
        ...
        // 根据标准头的类型字段计算消息头的总大小
        // 包括存储头、标准头、扩展头和标准头的额外大小，以便正确处理和发送日志消息
    ...
    // 计算消息的总长度：头部长度-存储头长度+日志消息体长度
    // 确保消息长度合法且不会超过最大值，以便正确处理和发送日志消息
    ...
    len = (uint32_t)tmplen;
    // 将消息长度转换为大端格式，并设置在标准头的长度字段中，以便正确标识和处理日志消息
    msg.standardheader->len = DLT_HTOBE_16(len);

    /* print to std out, if enabled */
    // 根据用户配置决定是否将日志消息打印到标准输出，如果启用，则调用相应的函数进行打印，以便在本地查看日志消息
    ...
    if (dlt_user.dlt_is_file) {
        ...
        if (dlt_user_file_reach_max) {
            ...
            if (msg_size > dlt_user.filesize_max) {
                ...
                /* log to file */
                // 否则，继续将日志消息（消息头buffer和日志消息体）写入文件，以便正确处理和发送日志消息
        ...
        // 如果日志消息不是写入文件
        // 检查是否有之前未发送成功的消息，如果有，则尝试重新发送这些消息，并在成功后清除溢出计数器，以便正确处理和发送日志消息
        ...
        // 如果日志消息的句柄有效且应用程序ID不为空，则尝试重新发送之前未成功发送的消息，以便确保日志消息能够正确处理和发送
        ...
        // 如果之前的消息重新发送成功或者没有需要重新发送的消息，并且应用程序ID不为空，则继续发送当前的日志消息；
        if ((ret == DLT_RETURN_OK) && (dlt_user.appID[0] != '\0')) {
        /* resend ok or nothing to resent */
// 判断是否启用了共享内存功能
// 共享内存：dlt_shm -> DltBuffer(ring buffer) -> log message
#ifdef DLT_SHM_ENABLE

            if (dlt_user.dlt_log_handle != -1)
            // 如果启用，则将日志消息（消息头buffer和日志消息体）推送到共享内存中，以便其他进程能够访问和处理日志消息；
                dlt_shm_push(&dlt_user.dlt_shm, msg.headerbuffer + sizeof(DltStorageHeader),
                             msg.headersize - sizeof(DltStorageHeader),
                             log->buffer, log->size, 0, 0);
            // 此时调用函数只将用户头发送到DLT守护进程，日志消息传则在共享内存
...
#else
// 判断是否启用了测试功能
// 如果启用，则根据用户配置可能会故意破坏用户消息头或消息大小，以便测试日志消息的处理和发送功能的健壮性和错误处理能力；
#   ifdef DLT_TEST_ENABLE
...
#endif
// 没有启用共享内存功能，则直接调用函数将用户头、消息头和日志消息体发送到DLT守护进程，以便正确处理和发送日志消息
// 参数分析：
//  msg.headerbuffer + sizeof(DltStorageHeader)：指针偏移到 StandardHeader 的起始位置，跳过存储头，指向真正要发送的标准头部分
//  实际要发送的大小 = 总大小 - StorageHeader（headersize 包含了所有头部，包括 StorageHeader）
//  原因：StorageHeader 仅用于本地文件存储，IPC 通信给守护进程时不需要
//  这样设计的好处是 StorageHeader 作为"包装层"，只在保存到文件时使用，而网络传输和进程间通信时可以省掉这 16 字节的开销
            ret = dlt_user_log_out3(dlt_user.dlt_log_handle,
                                    &(userheader), sizeof(DltUserHeader),
        ...
        // 根据发送日志消息的结果进行错误处理
        // 如果发送失败，则根据不同的错误类型进行相应的处理
        // 如关闭文件描述符、释放共享内存、打印日志消息等，以便正确处理和发送日志消息
        ...
        if ((ret != DLT_RETURN_OK) || (dlt_user.appID[0] == '\0'))
#endif  
// 如果发送日志消息失败或者应用程序ID为空，则调用错误处理函数，
// 将用户头、消息头和日志消息体传递给错误处理函数，以便正确处理和记录错误情况
        ...
        // 如果错误处理函数成功处理了错误情况，则继续执行后续操作息
        if (process_error_ret == DLT_RETURN_OK) {
            dlt_mutex_unlock();
            return DLT_RETURN_OK;
        }
        // 缓冲区已满，则增加溢出计数器并返回相应的错误码
        ...
        // 根据错误处理函数的返回值进行进一步的错误处理
        switch (ret) {
            // 场景1：PIPE 已满，则增加溢出计数器并返回相应的错误码
            case DLT_RETURN_PIPE_FULL:
            {
                /* data could not be written */
                dlt_mutex_unlock();
                return DLT_RETURN_PIPE_FULL;    // ← 直接返回，消息已在 startup_buffer 中
            }
            // 场景2：PIPE 错误 / Socket 关闭 / Daemon 断开
            case DLT_RETURN_PIPE_ERROR:
            {
                /* handle not open or pipe error */
                // 步骤1: 关闭file descriptor
                close(dlt_user.dlt_log_handle);
                dlt_user.dlt_log_handle = -1;   // ← 标记为无效

                // 步骤2: 标记需要重新连接（针对 Socket/VSOCK）
#if defined DLT_LIB_USE_UNIX_SOCKET_IPC || defined DLT_LIB_USE_VSOCK_IPC
            dlt_user.connection_state = DLT_USER_RETRY_CONNECT;
#endif
            // 步骤3: 释放共享内存资源（如果启用SHM）
    #ifdef DLT_SHM_ENABLE
            /* free shared memory */
            dlt_shm_free_client(&dlt_user.dlt_shm);
    #endif
...
// This function is used to send log messages through the DLT user library with version 2 of the DLT protocol
// 额外参数： msgcontent - 消息内容类型，指定日志消息的详细程度和格式，以便正确处理和发送日志消息
DltReturnValue dlt_user_log_send_log_v2(DltContextData *log, const int mtype, DltHtyp2ContentType msgcontent, int *const sent_size)
{
    ...
    // 为消息头分配内存，大小为计算得到的消息头总大小，以便正确处理和发送日志消息
    ...
    // 设置基础头的指针，指向存储头之后的位置，以便正确填充和处理基础头信息
    ...
    // 新增的基础头类型标志，表示是否包含应用程序ID和上下文ID，以便在不同配置下正确处理日志消息
    ...
    // 判断额外参数的内容类型，如果是详细数据消息，
    // 则根据消息类型设置基础头的MSIN字段，并根据当前的详细模式设置标志，以便正确标识和处理日志消息
...
*/
    ...
    /* Fill out extended header, if extended header should be provided */
    // 判断标志位是否启用WEID（ECU ID），如果启用，则设置ECU ID的长度和指针，以便正确标识和处理日志消息
    ...
    // 判断标志位是否包含WACID(App/Context ID)，如果启用，则设置应用程序ID和上下文ID的长度和指针，以便正确标识和处理日志消息
    ...
    // 判断标志位是否包含WSID，如果启用，则设置会话ID为当前进程ID，以便正确标识和处理日志消息
    ...
    // 判断标志位是否包含WSFLN(源文件名和行号)，如果启用，则设置源文件名长度和指针，以及行号，以便正确标识和处理日志消息
    ...
    // 判断标志位是否包含WTGS(标签)，如果启用，则设置标签的数量和指针，以便正确标识和处理日志消息
    ...
    */
    ...
    // 根据用户配置决定日志消息的输出方式，如果配置为写入文件，
    // 则检查文件大小是否超过最大限制，并将日志消息写入文件；
    if (dlt_user.dlt_is_file) {
        if (dlt_user_file_reach_max) {
            return DLT_RETURN_FILESZERR;
        }
        ...
        if ((ret == DLT_RETURN_OK) && (dlt_user.appID2len != 0)) {
            /* resend ok or nothing to resent */
#ifdef DLT_SHM_ENABLE
// 如果启用了共享内存功能，并且日志消息的句柄有效，则将日志消息推送到共享内存中，以便其他进程能够访问和处理日志消息；
...
#   endif
// 如果启用了负载控制跟踪功能，并且日志消息尚未发送，则在输出日志消息之前检查负载控制，以确保日志消息的发送不会超过系统的负载限制；
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
            /* check trace load before output */
            if (!sent_size)
            {
...
#endif
// 如果没有启用共享内存功能，并且日志消息的句柄有效，则直接调用函数将用户头、消息头和日志消息体发送到DLT守护进程，以便正确处理和发送日志消息；
...
void dlt_user_log_reattach_to_daemon(void)
{
    ...
    // 检查是否需要重连（dlt_log_handle < 0 或连接已断开）
...
{
    DltReturnValue ret = DLT_RETURN_ERROR;
    size_t msg_size = len1 + len2 + len3;   // 计算消息的总大小
    ...
    if (dlt_buffer_push3(&(dlt_user.startup_buffer),
                         ptr1, (unsigned int)len1,  // User header
                         ptr2, (unsigned int)len2,  // Standard header + extended header
                         ptr3, (unsigned int)len3) == DLT_RETURN_ERROR) {   // Payload
        // 缓冲区已满 → 记录警告（仅第一次）
        if (dlt_user.overflow_counter == 0) // 避免重复打印
