...
// 该函数使用writev系统调用将三个数据块（ptr1、ptr2、ptr3）写入到指定的文件描述符（handle）中，
// 并根据写入结果返回相应的状态码，以便正确处理日志消息的发送和错误处理
DltReturnValue dlt_user_log_out3(int handle, void *ptr1, size_t len1, void *ptr2, size_t len2, void *ptr3, size_t len3)
{
    ...
    // StorageHeader 不送到守护进程（仅本地文件存储）
    iov[0].iov_base = ptr1; // UserHeader (5 字节)
    iov[0].iov_len = len1;
    iov[1].iov_base = ptr2; // StandardHeader + ExtendedHeader
    iov[1].iov_len = len2;
    iov[2].iov_base = ptr3; // 消息负载
    iov[2].iov_len = len3;
    ...
    if (bytes_written != (len1 + len2 + len3)) {
        ...
        case EBADF:     // EBADF：无效文件描述符（handle=-1）
        {
            return DLT_RETURN_PIPE_ERROR;     /* EBADF - handle not open */
            break;
        }
        case EPIPE:     // EPIPE：守护进程断开 → 重新连接
        {
            return DLT_RETURN_PIPE_ERROR;     /* EPIPE - pipe error */
            break;
        }
        case EAGAIN:    // 缓冲满 → 存储到 Ring Buffer 后续重试
        {
